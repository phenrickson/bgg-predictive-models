...
def test_create_bgg_preprocessor_model_types(sample_dataframe):
    ...
    assert core_features.issubset(linear_columns), "Core features missing in linear model"
...
def test_bgg_preprocessor_feature_scaling(sample_dataframe):
    ...
    for feature in scaled_features:
        ...
        assert np.isclose(feature_data.mean(), 0, atol=1e-2), f"{feature} mean should be close to 0"
        assert np.isclose(feature_data.std(), 1, atol=1e-2), f"{feature} std should be close to 1"
...
def test_bgg_preprocessor_log_transformation(sample_dataframe):
    ...
    log_columns = ['min_age', 'min_playtime', 'max_playtime', 'time_per_player', 'description_word_count']
    ...
    processed_dataframe = linear_preprocessor.named_steps['bgg_preprocessor'].fit_transform(sample_dataframe)
    ...
    for feature in log_columns:
        ...
        assert f'{feature}' in linear_transformed.columns, f"Log transformation missing for {feature}"
        ...
        assert abs(log_transformed_skew) < abs(original_skew), f"Log transformation did not reduce skewness for {feature}"
...
def test_bgg_preprocessor_variance_selection():
    ...
    sample_dataframe = pd.read_parquet('data/raw/game_features.parquet').sample(n=10000, random_state=42)
    ...
    assert core_features.issubset(linear_transformed.columns), "Core features missing in linear model"
    assert core_features.issubset(tree_transformed.columns), "Core features missing in tree model"
