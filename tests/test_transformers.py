...
def test_time_per_player(sample_dataframe):
    ...
    for idx, row in sample_dataframe.iterrows():
        ...
        expected = (
            np.nan
            if row["max_playtime"] == 0 or row["max_players"] == 0
            else row["max_playtime"] / row["max_players"]
        )
...
def test_description_word_count(sample_dataframe):
    ...
    for idx, row in sample_dataframe.iterrows():
        expected_count = (
            len(str(row["description"]).split()) if pd.notna(row["description"]) else 0
        )
...
def test_feature_names_generation(sample_dataframe):
    ...
    transformer_partial = BaseBGGTransformer(
        create_category_features=False, create_family_features=False
    )
...
def test_feature_generation_flags(sample_dataframe):
    ...
    expected_columns = ["year_published"]
...
def test_feature_flag_with_min_max_features(sample_dataframe):
    ...
    for case in test_cases:
        ...
        feature_names = transformer.get_feature_names_out()
        assert not any(
            name.startswith(case["excluded_prefix"]) for name in feature_names
...
def test_categorical_feature_generation(sample_dataframe):
    ...
    designer_columns = [
        col for col in transformed_none.columns if col.startswith("designer_")
    ]
    artist_columns = [
        col for col in transformed_none.columns if col.startswith("artist_")
    ]
    publisher_columns = [
