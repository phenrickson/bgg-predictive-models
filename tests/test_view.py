...
def test_games_features_view_structure(bigquery_config):
    ...
    expected_columns = [
        'game_id', 'year_published', 'average_rating', 'average_weight',
        'users_rated', 'min_players', 'max_players', 'min_playtime',
        'max_playtime', 'min_age', 'image', 'thumbnail', 'description',
        'categories', 'mechanics', 'publishers', 'designers', 'artists',
        'families'
    ]
    ...
    array_columns = ['categories', 'mechanics', 'publishers', 'designers', 'artists', 'families']
    for col in array_columns:
        assert isinstance(result[col][0].to_list(), list), f"Column {col} is not an array"
...
def test_data_loader_training_data(data_loader):
    ...
    features, targets = data_loader.load_training_data(end_train_year=2022, min_ratings=1000)
    ...
    assert any(col.startswith('category_') for col in features.columns), "No category features found"
    assert any(col.startswith('mechanic_') for col in features.columns), "No mechanic features found"
...
def test_data_loader_prediction_data(data_loader):
    ...
    assert 'playtime_range' in features.columns, "Derived feature 'playtime_range' missing"
