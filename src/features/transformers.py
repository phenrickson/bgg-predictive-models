...
class LogTransformer(BaseEstimator, TransformerMixin):
    ...
    def set_output(self, *, transform=None):
        ...
        if transform is not None and transform not in ['default', 'pandas']:
            raise ValueError(f"Invalid transform parameter: {transform}. Must be 'default' or 'pandas'.")
...
class YearTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, reference_year: int = 2000, normalization_factor: int = 25):
        ...
        self.to_drop_ = ['year_published', 'year_published_centered', 'year_published_normalized']
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X_copy['year_published_centered'] = (X_copy['year_published'] - self.reference_year).astype(np.float64)
        X_copy['year_published_normalized'] = ((X_copy['year_published'] - self.reference_year) / self.normalization_factor).astype(np.float64)
...
def bgg_numeric_transformer(numeric_columns=None):
    ...
    numeric_transformer = ColumnTransformerNoPrefix([
        ('imputer', SimpleImputer(strategy='median', add_indicator=True, keep_empty_features=False), numeric_columns),
    ...
    def _create_mechanics_count(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def count_mechanics(mechanics):
            ...
            if mechanics is None:
                ...
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(f"Unexpected mechanics type: {type(mechanics)}, value: {mechanics}")
                return 0
