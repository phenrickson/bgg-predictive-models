...
def calculate_bayesian_average(games, dummy_ratings, dummy_average):
    ...
    est_bayesaverage = ((games['rating'] * games['users_rated']) + 
                        (dummy_ratings * dummy_average)) / (games['users_rated'] + dummy_ratings)
...
def plot_metrics(metrics_df):
    ...
    for avg in metrics_df['dummy_average'].unique():
        ...
        plt.plot(subset['dummy_ratings'], subset['rmse'], 
                 label=f'Avg: {avg:.1f}')
    ...
    for avg in metrics_df['dummy_average'].unique():
        subset = metrics_df.filter(pl.col('dummy_average') == avg)
        plt.plot(subset['dummy_ratings'], subset['mae'], 
                 label=f'Avg: {avg:.1f}')
    ...
    for avg in metrics_df['dummy_average'].unique():
        subset = metrics_df.filter(pl.col('dummy_average') == avg)
        plt.plot(subset['dummy_ratings'], subset['r2'], 
                 label=f'Avg: {avg:.1f}')
