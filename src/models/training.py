...
def load_data(
    ...
    if local_data_path:
        try:
            ...
            required_columns = ['year_published']  # Add other required columns as needed
...
def create_data_splits(
    ...
    try:
        ...
        if len(tune_df) == 0:
            raise ValueError(f"Tuning set is empty. Check tune years: {tune_start_year}-{tune_end_year}")
        if len(test_df) == 0:
            raise ValueError(f"Test set is empty. Check test years: {test_start_year}-{test_end_year}")
