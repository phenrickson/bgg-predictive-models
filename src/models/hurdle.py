...
def find_optimal_threshold(
    ...
    scoring_functions = {
        ...
        "f2": lambda y_true, y_pred: fbeta_score(
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--local-data", type=str, help="Path to local parquet file for training data"
    )
    parser.add_argument(
        "--model",
        type=str,
...
def main():
    ...
    train_metrics = evaluate_model(
        train_pipeline, train_X, train_y, "training", threshold=0.5
    )  # Use default threshold for initial evaluation
    ...
    tune_metrics = evaluate_model(
        train_pipeline, tune_X, tune_y, "tuning", threshold=optimal_threshold
    )
    ...
    test_metrics = evaluate_model(
        final_pipeline, test_X, test_y, "test", threshold=optimal_threshold
    )
