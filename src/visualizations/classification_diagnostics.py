...
def plot_confusion_matrix(
    ...
    """
    if 'actual' not in predictions_df.columns or 'prediction' not in predictions_df.columns:
...
def plot_roc_curve(
    ...
    Returns:
        Matplotlib figure and axes
    """
    if 'actual' not in predictions_df.columns or 'predicted_proba_class_1' not in predictions_df.columns:
        raise ValueError("DataFrame must contain 'actual' and 'predicted_proba_class_1' columns")
...
def main():
    ...
    predictions_df = pl.read_parquet('models/experiments/hurdle/test-hurdle/v2/test_predictions.parquet')
