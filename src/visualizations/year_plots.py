...
def plot_year_transformations(transformed_df: pl.DataFrame, output_path: str = 'figures/year_transformations.png'):
    """
    ...
    plt.subplot(2, 3, 2)
    plt.hist(transformed_df['year_published_centered'].to_numpy(), bins=50, edgecolor='black')
    ...
    plt.subplot(2, 3, 3)
    plt.hist(transformed_df['year_published_normalized'].to_numpy(), bins=50, edgecolor='black')
    ...
    plt.subplot(2, 3, 4)
    plt.hist(transformed_df['year_published_log_distance'].to_numpy(), bins=50, edgecolor='black')
    ...
    plt.subplot(2, 3, 5)
    plt.hist(transformed_df['year_published_quadratic'].to_numpy(), bins=50, edgecolor='black')
    ...
    plt.subplot(2, 3, 6)
    plt.hist(transformed_df['year_published_cubic'].to_numpy(), bins=50, edgecolor='black')
...
def plot_year_era_distribution(transformed_df: pl.DataFrame, output_path: str = 'figures/year_era_distribution.png'):
    """
    ...
    plt.bar(era_counts_sorted['year_published_era'].to_numpy(), era_counts_sorted['count'].to_numpy())
...
def generate_year_visualizations(raw_features: pl.DataFrame, 
                                 year_transformer=None, 
    ...
    plot_year_transformations(
        transformed_df, 
        os.path.join(output_dir, 'year_transformations.png')
    )
    
    # Plot era distribution
    era_counts = plot_year_era_distribution(
        transformed_df, 
        os.path.join(output_dir, 'year_era_distribution.png')
    )
