...
def create_complexity_predictions_plot(predictions_path: str, experiment_dir: str):
    ...
    df["prediction_error_percentage"] = (
        np.abs(df["prediction_error"] / df["true_complexity"]) * 100
    )
    ...
    rmse = np.sqrt(
        mean_squared_error(df["true_complexity"], df["predicted_complexity"])
    )
...
def main():
    ...
    if not predictions_files:
        raise FileNotFoundError(
            "No complexity predictions file found in model experiments directory"
        )
