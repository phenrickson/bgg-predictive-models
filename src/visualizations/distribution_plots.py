...
def plot_distributions(df):
    ...
    plt.subplot(2, 2, 1)
    sns.histplot(df.select(pl.col('complexity').drop_nulls()).to_numpy().flatten(), kde=True)
    ...
    # Rating distribution
    plt.subplot(2, 2, 2)
    sns.histplot(df.select(pl.col('rating').drop_nulls()).to_numpy().flatten(), kde=True)
    ...
    plt.subplot(2, 2, 3)
    sns.histplot(df.select(pl.col('users_rated').drop_nulls()).to_numpy().flatten(), kde=True)
    ...
    plt.subplot(2, 2, 4)
    log_users_rated = np.log1p(df.select(pl.col('users_rated').drop_nulls()).to_numpy().flatten())
