...
fi_groups = sorted(plot_partial_effects_by_group(fi_pdf).keys())


def _ggplot_to_img_html(plot, width: int = 9, height: int = 8) -> str:
    """Render a plotnine ggplot to an embedded base64 PNG.

    Plotnine's `save` accepts a buffer; we keep the bytes in memory so
    the qmd doesn't have to manage file paths or output dirs.
    """
    buf = BytesIO()
    plot.save(buf, format="png", width=width, height=height, dpi=96, verbose=False)
...
for group in fi_groups:
    ...
    print(f"### {group}\n")
    print("```{=html}")
    print(_ggplot_to_img_html(plot, width=9, height=6))
    print("```\n")
print(":::\n")
```
