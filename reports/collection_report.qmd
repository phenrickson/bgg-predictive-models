...
## Partial Effects (faceted grid)

The same coefficients, broken out by feature family — all panels in one
faceted figure. Static, smaller file size.
...
fi_groups = sorted(
    {g for g in (
        plot_partial_effects_by_group(fi_pdf).keys()
    )}
)
```

```{python}
#| label: partial-effects-grid
#| fig-width: 11
#| fig-height: 11
plot_feature_importance_grid(
    fi_pdf,
    ...
    top_neg=12,
    interactive=False,
    title="Partial effects by feature family",
)
```

## Partial Effects (tabset, interactive)

The same coefficients in plotly tabs — interactive but adds substantial
bytes per panel. Compare against the faceted grid above.

```{python}
#| label: partial-effects-build-interactive
#| include: false
plots = plot_partial_effects_by_group(fi_pdf)
```
