...
# Build two parallel pivots: one for the displayed game name and one
# for the underlying ownership label, so we can tint each cell by truth.
...
if _eval_view.height > 0:
    ...
    _name_pivot = _name_pivot.select(
        ["rank"] + [str(y) for y in _years if str(y) in _name_pivot.columns]
    )
    name_pdf = _name_pivot.to_pandas().set_index("rank").fillna("")
    ...
    if "label" in _eval_view.columns:
        ...
        _label_pivot = _label_pivot.select(
            ["rank"] + [str(y) for y in _years if str(y) in _label_pivot.columns]
        )
        label_pdf = _label_pivot.to_pandas().set_index("rank")
    ...
    styler = name_pdf.style.apply(_style_owned, axis=None).set_properties(
        **{"text-align": "left"}
    )
    column_defs = [
        {"width": "180px", "targets": "_all"},
    ]
    itables_show(
        styler,
        paging=False,
